...
def build_profiles(
    ...
    if active_top is not None:
        # cart+purchase 건수 상위 N명 유저만 — CF / content 섹션 데모 품질용
        activity = (
            df[df["event_type"].isin(["cart", "purchase"])]
            .groupby("user_id").size()
        )
        keep_uids = set(activity.nlargest(active_top).index)
        df = df[df["user_id"].isin(keep_uids)].copy()
        print(f"  active-top {active_top}: {len(keep_uids):,} users kept")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--active-top", type=int, default=None,
                        help="cart+purchase 상위 N명만 사용 (--max-users로 좁힌 후 추가 필터)")
    ...
    print("프로필 집계 중...")
    profiles = build_profiles(args.train, aliases,
                              max_users=args.max_users, active_top=args.active_top)
