...
EVENT_WEIGHT = {"view": 0.05, "cart": 1.0, "purchase": 2.0}
TOP_K = 20
...
def main(argv: list[str] | None = None) -> int:
    ...
    # zero-norm(=purchase/cart 0건) 유저 필터 — FAISS 이웃 품질 보장
    norms = np.linalg.norm(vectors, axis=1)
    active_mask = norms > 0
    n_active = int(active_mask.sum())
    print(f"  active(p+c>0): {n_active:,} / inactive 제거: {len(profiles) - n_active:,}")
    profiles = [p for p, m in zip(profiles, active_mask) if m]
    vectors = vectors[active_mask]
