...
실행:
    streamlit run service/ui/app.py

사전 조건 (service/data/ 안에 존재):
    user_recommendations.json · item_catalog.json
...
# .env 로드 (UPSTAGE_API_KEY 등) - service/.env 기준
...
from ui.components import evidence_chips
...
def _sort_aliases_by_activity(aliases: tuple[str, ...], db_path: str) -> list[str]:
    """pack_index 유저들을 purchase+cart 건수 내림차순으로 정렬 (CF 데모 품질 ↑)."""
    if not aliases:
        return []
    con = sqlite3.connect(db_path)
    placeholders = ",".join("?" for _ in aliases)
    rows = con.execute(
        f"SELECT user_alias, purchased_items, carted_items "
        f"FROM user_profiles WHERE user_alias IN ({placeholders})",
        list(aliases),
    ).fetchall()
    con.close()
    counts = {
        ua: len(json.loads(p or "[]")) + len(json.loads(c or "[]"))
        for ua, p, c in rows
    }
    # profile_db에 없거나 활동 0인 유저는 dropdown에서 제외 (추천 못 함)
    return sorted(
        [a for a in aliases if counts.get(a, 0) > 0],
...
def main() -> None:
    ...
    with st.sidebar:
        st.markdown("### 데모 유저")
        # evidence_pack 유저를 활동량(purchase+cart) 내림차순 정렬 → CF 섹션 데모 품질 ↑
        pack_aliases = tuple(sorted(resources.pack_index.keys()))
        if not pack_aliases:
            st.error("evidence_pack.jsonl에 유저가 없습니다. Phase 0b를 실행하세요.")
            return
        valid_aliases = _sort_aliases_by_activity(pack_aliases, str(resources.profile_db_path))
        selected = st.selectbox(
            "선택", valid_aliases[:50],
            help=f"evidence_pack {len(valid_aliases)}명 중 활동 많은 상위 50명",
        )
        st.markdown("---")
