...
def apply_hard_gate(pack: "EvidencePack", response: "AdvisorResponse") -> "AdvisorResponse":
    """검사 결과를 response에 채워서 돌려줌. response는 변형됨(in-place)."""
    failures = check_response(pack, response)
    response.hard_gate_passed = len(failures) == 0
    if failures:
        response.flags = list(response.flags) + [f"hard_gate: {f}" for f in failures]
    return response
...
if __name__ == "__main__":
    ...
    from advisor.schema import AdvisorResponse, Claim
    ...
    apply_hard_gate(pack, good)
    print(f"GOOD: passed={good.hard_gate_passed} flags={good.flags}")

    # 실패 케이스 — 일부러 깨진 응답 만들어보기
    bad = AdvisorResponse(
        user_id=pack.user_id, item_id="non-existent-item",  # 범위 밖
        question_key="why", summary="가짜",
        claims=[Claim(text="없는 근거", evidence_ref="signals.fake_field")],
        decision_hint="", confidence_raw=0.99,
    )
    apply_hard_gate(pack, bad)
    print(f"BAD: passed={bad.hard_gate_passed} flags={bad.flags}")
