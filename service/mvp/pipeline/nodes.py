...
from advisor.prompts import QUICK_QUESTIONS, classify_intent
...
class ResourceBundle:
    ...
    def load(
        ...
        if neighbors_path.exists() and alias_to_row_path.exists() and row_to_alias_path.exists():
            ...
            cf_neighbors = {
                alias: [row_to_alias[r] for r in neighbors[row]]
...
def make_node_alias_resolver(_resources: ResourceBundle):
    def node(state: GraphState) -> GraphState:
        ...
        if state.get("user_alias"):
            return {}
        return {"response_text": "추천을 위해 user_00001 형식의 ID가 필요합니다.",
...
def make_node_profile_loader(resources: ResourceBundle):
    def node(state: GraphState) -> GraphState:
        ...
        if not ua:
            return {"response_text": f"유저 별칭이 없습니다.", "trust_badge": "unverified"}
...
def make_node_general_chat(_resources: ResourceBundle):
    def node(state: GraphState) -> GraphState:
        msg = state.get("message", "")
        # MVP: general 모드는 Evidence 없이 LLM 직접 호출 (또는 mock 안내)
        # 실제 Solar Pro 호출은 generate() API 시그니처 차이 때문에 advisor 외부에서 처리
        # 여기선 안내 메시지만
        return {
            "response_text": (
...
def route_after_alias(state: GraphState) -> str:
    # alias 없으면 (이미 response_text 설정됨) END
