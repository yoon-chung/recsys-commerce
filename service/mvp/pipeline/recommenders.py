"""recommenders.py — 4종 추천 엔진 (Python 규칙 기반, LLM 호출 없음).

Phase 3에서 build_rag_index.py 이후 호출하거나, 별도 파이프라인에서 사용.
각 엔진은 item_alias 리스트를 반환한다 — Evidence Pack 빌드 전에 후보를 고정.
"""
...
def recommend_submission(
    ...
    """submission 기반 Top-N 반환. build_rag_index 출력 dict 그대로 사용."""
...
def recommend_revisit(
    ...
    """
    TIFU 스타일 재방문 후보 생성.
    side_project/evidence_pack/builder.py의 recommend_revisit()를 호출한다.

    event_log 포맷:
        [{"item_id": item_alias, "event_type": "view"|"cart"|"purchase", "days_ago": int}, ...]
    """
    from evidence_pack.builder import recommend_revisit as _core

    return _core(event_log, top_n=top_n)
