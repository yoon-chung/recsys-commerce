...
class CSVAdapter:
    ...
    예: 0.1504 기록 본체 형식과 호환 — `cart_boost: false` 여부와 무관하게 CSV 컬럼만 맞으면 OK.
    rich signals는 CSV만으론 알 수 없으므로 None. 본체가 별도 feature CSV를 같이 dump하면
    별도 어댑터(TeamAdapter 등)를 추가하면 된다.
    """
    ...
    def __init__(self, submission_csv: str | Path, top_k: int = 10, max_users: int | None = None):
        ...
        self._top_k = top_k
        self._top: dict[str, list[str]] = {}
        # 입력 순서 = 순위 가정 (validate_submission이 보장). max_users가 있으면
        # submission의 user-grouped top-K prefix만 읽어 부분 빌드 비용을 낮춘다.
    ...
    def get_rich_signals(self, user_id: str) -> dict | None:
        return None  # 풍부한 신호는 별도 어댑터에서


# ---------- AliasCSVAdapter (Phase 2 연결용) ----------------------------------


class AliasCSVAdapter:
    """Phase 2 완성 후 사용. alias 기반 submission CSV를 CSVAdapter처럼 제공.

    id_alias.py가 생성한 id_aliases.json을 읽어,
    submission CSV의 UUID를 alias로 변환해서 Evidence Pack 빌드에 전달한다.

    TODO: build_rag_index.py에서 alias 주입 로직을 분리하고 싶을 때 이 어댑터를 사용.
    ...
    def __init__(
        ...
        self._inner = CSVAdapter(submission_csv, top_k=top_k, max_users=max_users)
        users = self._aliases.get("users", {})
        items = self._aliases.get("items", {})

        # UUID 기반 top-K를 alias 기반으로 변환
        self._top: dict[str, list[str]] = {}
        for uuid_uid, item_uuids in (
            (uid, self._inner.get_top_k(uid)) for uid in self._inner.list_user_ids()
        ):
            alias_uid = users.get(uuid_uid, uuid_uid)
            alias_items = [items.get(iid, iid) for iid in item_uuids]
            self._top[alias_uid] = alias_items
...
class ParquetCatalogSource:
    ...
    def _prepare_events(df):
        ...
        # brand demo 재매핑 (Rees46 dataset의 electronics brand → fictional fashion 명)
        try:
            from pipeline.brand_remap import remap_brand_series
            df["brand"] = remap_brand_series(df["brand"])
        except ImportError:
            pass  # pipeline 모듈 없는 환경 (e.g., 단독 evidence_pack test)
        return df
