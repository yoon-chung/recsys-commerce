"""exp_009_mbstr / train.py -- BSARec + behavior embedding (lightweight MB-STR).

exp_006 BSARec+CL hybrid 와 동일 패턴. 모델 클래스만 MBSTR.
...
def main() -> None:
    ...
    logger.info("MBSTR model -- %s params (%.2fM)", f"{n_params:,}", n_params / 1e6)
    logger.info("  alpha=%.2f c=%d  +  n_behaviors=%d",
                config["alpha"], config["c"], config["n_behaviors"])
