"""exp_006_bsarec_cl / train.py -- BSARec + CL4SRec hybrid via RecBole.

Same driver pattern as exp_002_bsarec / exp_002b. Only the model class
changes: BSARecCL extends BSARec by adding a CL4SRec InfoNCE auxiliary
loss over crop/mask/reorder augmented views (see bsarec_cl_model.py).

The contrastive component only fires during training; inference is
identical to vanilla BSARec (same forward + matmul-with-item-embedding).

Artifacts -> ./saved/ (gitignored):
    BSARecCL-<timestamp>.pth
    wandb_run_id.txt
    best_ckpt_path.txt
"""
...
# bsarec_cl_model -> bsarec_model -> fra (all siblings in HERE).
from bsarec_cl_model import BSARecCL  # noqa: E402
...
def main() -> None:
    ...
    model = BSARecCL(config, train_data.dataset).to(config["device"])
    n_params = sum(p.numel() for p in model.parameters())
    logger.info("BSARecCL model -- %s params (%.2fM)", f"{n_params:,}", n_params / 1e6)
    logger.info("  alpha=%.2f c=%d  +  lmd=%.2f tau=%.2f crop=%.2f mask=%.2f reorder=%.2f",
                config["alpha"], config["c"],
                config["lmd"], config["tau"],
                config["crop_ratio"], config["mask_ratio"], config["reorder_ratio"])
