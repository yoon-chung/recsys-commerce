"""exp_002d_bsarec_1w / data_prep.py -- build 1-week RecBole atomic dataset.

Same as exp_002b_bsarec_4w/data_prep.py except `--last-days` defaults to 7
(last 1 week of train data, Feb 16-22 before fixed Feb 23-29 val), and
output goes to ./data/cy_commerce_1w/.

This file is gitignored via the global `**/data/` pattern.

Usage:
    python data_prep.py                       # default: last 7 days
"""
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=7,
        help="keep only the last N days of train_df (1-week recency window). "
             "default 7 = Feb 16-22 (train side; val Feb 23-29 is fixed).",
    )
    parser.add_argument("--out-dir", default=str(HERE / "data" / "cy_commerce_1w"))
    parser.add_argument("--dataset-name", default="cy_commerce_1w")
