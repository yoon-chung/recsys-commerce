"""ensemble_v2_als_ease_bsarec / run.py — RRF combine of ALS + EASE + BSARec.

Goal: test family-diversity hypothesis with 3 truly different families
(MF factor vs item-item vs sequential). ensemble_v1 was negative because
ALS + EASE were both CF family; v2 adds the sequential BSARec which proved
to capture genuinely different signal (self-val 0.2391 vs ALS/EASE 0.184).

Inputs (relative to this script):
    ../exp_000_als_baseline/predictions.parquet
    ../exp_001_ease/predictions.parquet
    ../exp_002_bsarec/predictions.parquet
    ../exp_001_ease/saved/val_gt.parquet          (shared split)
    ../exp_001_ease/saved/eval_users.json
    ../exp_001_ease/saved/mappings/
...
def main() -> None:
    ...
    parser.add_argument(
        "--bsarec-pred",
        default=str(here.parent / "exp_002_bsarec" / "predictions.parquet"),
    )
    parser.add_argument(
        "--ease-saved",
    ...
    logger.info("loading BSARec predictions: %s", args.bsarec_pred)
    pred_bsarec = pd.read_parquet(args.bsarec_pred)
    logger.info("  BSARec %s rows, %s users", f"{len(pred_bsarec):,}",
                f"{pred_bsarec['user_id'].nunique():,}")
    ...
    bsarec_ndcg10 = ndcg_at_k_from_df(pred_bsarec, val_gt_eval, k=10)
    bsarec_recall10 = recall_at_k_from_df(pred_bsarec, val_gt_eval, k=10)
    ...
    best_single_ndcg = max(als_ndcg10, ease_ndcg10, bsarec_ndcg10)
    best_single_recall = max(als_recall10, ease_recall10, bsarec_recall10)
    logger.info("RRF 3-model self-val:")
    logger.info("  fused NDCG@10   = %.6f  (best single = %.6f BSARec, delta %+.6f)",
                fused_ndcg10, best_single_ndcg, fused_ndcg10 - best_single_ndcg)
    logger.info("  fused recall@10 = %.6f  (best single = %.6f, delta %+.6f)",
                fused_recall10, best_single_recall, fused_recall10 - best_single_recall)
    logger.info("  vs each:")
    logger.info("    ALS    NDCG delta %+.6f", fused_ndcg10 - als_ndcg10)
    logger.info("    EASE   NDCG delta %+.6f", fused_ndcg10 - ease_ndcg10)
            ...
            wandb.log(
                {
                    ...
                    "bsarec_ndcg@10": bsarec_ndcg10,
                    "bsarec_recall@10": bsarec_recall10,
                    ...
                    "lift_vs_bsarec_ndcg": fused_ndcg10 - bsarec_ndcg10,
                    "lift_vs_best_single_ndcg": fused_ndcg10 - best_single_ndcg,
                    "lift_vs_als_recall": fused_recall10 - als_recall10,
                    "lift_vs_ease_recall": fused_recall10 - ease_recall10,
                    "lift_vs_bsarec_recall": fused_recall10 - bsarec_recall10,
                }
            )
            run.finish()
        except ImportError:
