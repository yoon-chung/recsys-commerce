"""exp_010b_lgbm_lambdarank / inference.py -- 638k users LambdaRank ensemble.

exp_010 inference.py 와 동일 로직 (Booster.predict 호출 방식 동일).
다른 점: saved/ 의 fold model 이 LGBMRanker 출신 + features_dir 가 exp_010 의 cache 참조.
...
def main() -> None:
    ...
    features_path = Path(args.features) if args.features else (HERE / cfg["features_dir"]).resolve()
    ...
    for i, cp in enumerate(chunk_paths):
        ...
        logger.info("  chunk %d/%d: %s top-%d rows",
                    i + 1, len(chunk_paths), f"{len(out_c):,}", top_n)
    ...
    # ---- 4. Self-val NDCG (caveat: leaked, 진짜 OOF 는 train_cv.py 출력 참조) ----
    ...
    logger.info("self-val NDCG@10=%.6f recall@10=%.6f (CAVEAT: leaked, 진짜 측정은 OOF)",
                ndcg, recall)
    ...
    logger.info("baseline TIFU NDCG=%.6f  Δ vs TIFU NDCG: %+.6f",
                tifu_ndcg, ndcg - tifu_ndcg)
