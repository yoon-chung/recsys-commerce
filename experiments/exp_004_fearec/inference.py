"""exp_004_fearec / inference.py -- score 638k users via FEARec.

FEARec has a SASRec backbone with FFT + auto-correlation enhanced encoder.
Inference uses the same matmul-with-item-embedding pattern as
exp_002_bsarec -- forward returns seq_output [B, H], scored against
...
def main() -> None:
    ...
    if ckpt_pointer.exists():
        ...
        candidates = sorted(saved_dir.glob("FEARec-*.pth"))
        if not candidates:
            raise FileNotFoundError(f"no FEARec checkpoint under {saved_dir}")
