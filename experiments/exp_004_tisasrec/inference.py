"""exp_004_tisasrec / inference.py -- score 638k users via TiSASRec.

Same structure as exp_002_bsarec/inference.py, but TiSASRec needs the
per-sequence timestamp list (for its pairwise time-gap matrix) as an
additional input. We build (item_seq, time_seq, item_seq_len) per user
and feed via a RecBole Interaction so model.full_sort_predict handles the
time_matrix construction internally.

Outputs:
    predictions.parquet   user_id, item_id (UUID), score, rank   (top-50)
...
def build_user_seq_with_time(train_df: pd.DataFrame) -> dict:
    """Group sorted (item_uuid, unix_ts) history per user."""
    sorted_df = train_df.sort_values(["user_id", "event_time"], kind="mergesort")
    ts = (
        pd.to_datetime(sorted_df["event_time"], format="%Y-%m-%d %H:%M:%S %Z")
        .astype("int64") // 10**9
    ).to_numpy()
    sorted_df = sorted_df.assign(_ts=ts)
    out: dict = {}
    for user_id, group in sorted_df.groupby("user_id", sort=False):
        out[user_id] = (group["item_id"].tolist(), group["_ts"].tolist())
    return out
...
def main() -> None:
    ...
    if ckpt_pointer.exists():
        ...
        candidates = sorted(saved_dir.glob("TiSASRec-*.pth"))
        if not candidates:
            raise FileNotFoundError(f"no TiSASRec checkpoint under {saved_dir}")
    ...
    # Field names for sequential models in RecBole. These come from the
    # model's __init__: self.ITEM_SEQ, self.ITEM_SEQ_LEN, self.TIME_SEQ
    item_seq_field = model.ITEM_SEQ
    item_seq_len_field = model.ITEM_SEQ_LEN
    time_seq_field = getattr(model, "TIME_SEQ", "timestamp_list")
    logger.info("interaction fields -- item_seq=%s, len=%s, time_seq=%s",
                item_seq_field, item_seq_len_field, time_seq_field)

    # ---- 3. Our train portion + per-user (item, timestamp) sequences -----
    ...
    user_seqs = build_user_seq_with_time(train_df)
    ...
    known_time_seqs: list[list[int]] = []
    known_seq_lens: list[int] = []

    for u in all_users:
        if u not in user_seqs:
            continue
        items_uuid, ts_list = user_seqs[u]
        kept_items: list[int] = []
        kept_ts: list[int] = []
        for it, t in zip(items_uuid, ts_list):
            if it in item_field2id:
                kept_items.append(item_field2id[it])
                kept_ts.append(t)
        if not kept_items:
            continue
        kept_items = kept_items[-max_len:]
        kept_ts = kept_ts[-max_len:]
        L = len(kept_items)
        padded_items = kept_items + [pad_token] * (max_len - L)
        padded_ts = kept_ts + [0] * (max_len - L)
        known_users.append(u)
        known_seqs.append(padded_items)
        known_time_seqs.append(padded_ts)
    ...
    # ---- 6. Batched scoring via Interaction + full_sort_predict ----------
    ...
    with torch.no_grad():
        for start in range(0, n_known, batch_size):
            ...
            interaction_data = {
                ...
                time_seq_field: torch.tensor(known_time_seqs[start:end], dtype=torch.long),
            }
